...
Post traffic data to sales system
    ...
    Handle API traffic response    ${status}    ${return}    ${traffic_data}

Handle API traffic response
    [Arguments]    ${status}    ${return}    ${traffic_data}
    IF    "${status}" == "PASS"
        Handle API traffic OK response
    ELSE
        Handle API traffic ERROR response    ${return}    ${traffic_data}
    END

Handle API traffic OK response
    Release Input Work Item    DONE

Handle API traffic ERROR response
